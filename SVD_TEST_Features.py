...
with open("test_comb_tfidf_transform", "rb") as f:
    heads_and_bodies_tfidf = pkl.load(f)
with open("test_head_tfidf_transform", "rb") as f:
    head_tfidf_vectors = pkl.load(f)
with open("test_body_tfidf_transform", "rb") as f:
...
svd_headlines = svd_transform[:len(old_body_IDs)]
svd_bodies = svd_transform[len(old_body_IDs):]
...
with open("test_svd_sim", "wb") as f:
